...
# Only when nobody has said otherwise. A plain assignment here beats the CC the
# environment carries, which is how a cross build names its compiler - the
# buildbot's mingw jobs were being built with the host's gcc because of it.
ifeq ($(origin CC), default)
   CC = gcc
endif
